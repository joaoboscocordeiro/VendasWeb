...
public sealed class ProdutoCriadoConsumerBackgroundService : BackgroundService
{
    private static readonly JsonSerializerOptions JsonOptions = new(JsonSerializerDefaults.Web);

    private readonly IServiceScopeFactory _scopeFactory;
    private readonly RabbitMqOptions _rabbitMqOptions;
    private readonly ProdutoCriadoConsumerOptions _consumerOptions;
    private readonly ILogger<ProdutoCriadoConsumerBackgroundService> _logger;

    public ProdutoCriadoConsumerBackgroundService(
        IServiceScopeFactory scopeFactory,
        IOptions<RabbitMqOptions> rabbitMqOptions,
        IOptions<ProdutoCriadoConsumerOptions> consumerOptions,
        ILogger<ProdutoCriadoConsumerBackgroundService> logger)
    {
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            catch (Exception exception)
            {
                _logger.LogWarning(
                    exception,
                    "Falha ao consumir catalogo.produto-criado.v1. Nova tentativa em 10 segundos.");
    ...
    private async Task ConsumirAsync(CancellationToken stoppingToken)
    {
        ...
        async Task ProcessarEntregaAsync(object sender, BasicDeliverEventArgs entrega)
        {
            try
            {
                ...
                var evento = JsonSerializer.Deserialize<ProdutoCriadoEvento>(
                    entrega.Body.Span,
                    JsonOptions);
                ...
                var processador = scope.ServiceProvider.GetRequiredService<IProcessadorProdutoCriado>();
            ...
            catch (Exception exception)
            {
                _logger.LogError(
                    exception,
                    "Falha ao processar mensagem {RoutingKey}.",
                    entrega.RoutingKey);
